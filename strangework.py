...
def xee():
  ...
  profile_resp  = requests.get(spotify_profile_url, headers=authorization_header)
  ...
  playlist_query_resp = requests.get('{}/playlists'.format(profile_url), headers=authorization_header)
  ...
  return flask.render_template('xee.html', spotify_name=user_id, tracks=tracks, playlist_names=playlist_names)
...
def get_song_comments(id):
  ...
  #first_comment = {
  #  "user":"Me",
  #  "comment":"Ah! Song " +id+ "! This is a good song!"
  #}
  #second_comment = {
  #  "user":"Myself",
  #  "comment":"Yes! This a good song!"
  #}
  #comments.append(first_comment)
  #comments.append(second_comment)
...
def write_comment():
  ...
  conn = sqlite3.connect(XEE_COMMENTS_DB)
  c = conn.cursor()
  params = (comment_obj['song_id'], flask.session['spotify_user_id'], comment_obj['comment'])
